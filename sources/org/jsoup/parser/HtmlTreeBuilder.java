...
class HtmlTreeBuilder extends TreeBuilder {
    ...
    public List<Node> parseFragment(String inputFragment, Element context, String baseUri, ParseErrorList errors) {
        ...
        if (context != null) {
            ...
            if (StringUtil.m203in(contextTag, "title", "textarea")) {
                this.tokeniser.transition(TokeniserState.Rcdata);
            } else {
                if (StringUtil.m203in(contextTag, "iframe", "noembed", "noframes", "style", "xmp")) {
    ...
    public void popStackToClose(String... elNames) {
        Iterator<Element> it = this.stack.descendingIterator();
        while (it.hasNext()) {
            if (StringUtil.m203in(it.next().nodeName(), elNames)) {
                it.remove();
                return;
            }
            it.remove();
    ...
    private void clearStackToContext(String... nodeNames) {
        ...
        while (it.hasNext()) {
            Element next = it.next();
            if (!StringUtil.m203in(next.nodeName(), nodeNames) && !next.nodeName().equals("html")) {
                it.remove();
            } else {
                return;
            }
    ...
    private boolean inSpecificScope(String[] targetNames, String[] baseTypes, String[] extraTypes) {
        ...
        while (it.hasNext()) {
            ...
            if (StringUtil.m203in(elName, targetNames)) {
                return true;
            }
            if (StringUtil.m203in(elName, baseTypes)) {
                return false;
            }
            if (extraTypes != null && StringUtil.m203in(elName, extraTypes)) {
                return false;
            }
        }
    ...
    public boolean inSelectScope(String targetName) {
        ...
        while (it.hasNext()) {
            ...
            if (!StringUtil.m203in(elName, TagSearchSelectScope)) {
                return false;
            }
        }
        Validate.fail("Should not be reachable");
    ...
    public void generateImpliedEndTags(String excludeTag) {
        while (excludeTag != null && !currentElement().nodeName().equals(excludeTag) && StringUtil.m203in(currentElement().nodeName(), TagSearchEndTags)) {
            pop();
        }
    }
    ...
    public boolean isSpecial(Element el) {
        return StringUtil.m203in(el.nodeName(), TagSearchSpecial);
    }
