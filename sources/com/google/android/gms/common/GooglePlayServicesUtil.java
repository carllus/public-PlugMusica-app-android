...
public final class GooglePlayServicesUtil extends zze {
    ...
    private static void zza(int i, Context context, String str, PendingIntent pendingIntent) {
        ...
        if (zzi.zzcl(context)) {
            zzac.zzbr(zzs.zzaxo());
            build = new Notification.Builder(context).setSmallIcon(C0417R.C0418drawable.common_ic_googleplayservices).setPriority(2).setAutoCancel(true).setStyle(new Notification.BigTextStyle().bigText(new StringBuilder(String.valueOf(zzh).length() + 1 + String.valueOf(zzj).length()).append(zzh).append(" ").append(zzj).toString())).addAction(C0417R.C0418drawable.common_full_open_on_phone, resources.getString(C0417R.string.common_open_on_phone), pendingIntent).build();
        } else {
            String string = resources.getString(C0417R.string.common_google_play_services_notification_ticker);
            if (zzs.zzaxk()) {
