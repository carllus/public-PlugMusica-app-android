    ...
    public Looper zzuy() {
        ...
        synchronized (this.zzakd) {
            if (this.zzcsv != 0) {
                ...
                zzkn.m162v("Starting the looper thread.");
                this.zzcsu = new HandlerThread("LooperProvider");
                this.zzcsu.start();
                this.mHandler = new Handler(this.zzcsu.getLooper());
                zzkn.m162v("Looper thread started.");
            } else {
                zzkn.m162v("Resuming the looper thread");
                ...
                this.mHandler.post(new Runnable() {
                    public void run() {
                        synchronized (zzkz.this.zzakd) {
                            zzkn.m162v("Suspending the looper thread");
                            while (zzkz.this.zzcsv == 0) {
                                try {
                                    zzkz.this.zzakd.wait();
                                    zzkn.m162v("Looper thread resumed");
                                } catch (InterruptedException e) {
                                    zzkn.m162v("Looper thread interrupted.");
                                }
                            }
                        }
                    }
