...
public class EasyTracker extends Tracker {
    ...
    private void loadParameters() {
        ...
        if (!(logLevelString == null || (logLevel = getLogLevelFromString(logLevelString)) == null)) {
            Log.m23v("[EasyTracker] log level loaded: " + logLevel);
        ...
        Log.m23v("[EasyTracker] sample rate loaded: " + sampleRate);
        int dispatchPeriod = this.mParameterFetcher.getInt("ga_dispatchPeriod", 1800);
        Log.m23v("[EasyTracker] dispatch period loaded: " + dispatchPeriod);
        ...
        Log.m23v("[EasyTracker] session timeout loaded: " + this.mSessionTimeout);
        ...
        Log.m23v("[EasyTracker] auto activity tracking loaded: " + this.mIsAutoActivityTracking);
        ...
        if (isAnonymizeIpEnabled) {
            ...
            Log.m23v("[EasyTracker] anonymize ip loaded: " + isAnonymizeIpEnabled);
        }
        ...
        if (this.mIsReportUncaughtExceptionsEnabled) {
            ...
            Log.m23v("[EasyTracker] report uncaught exceptions loaded: " + this.mIsReportUncaughtExceptionsEnabled);
        }
    ...
    private void setContext(Context ctx, ParameterLoader parameterLoader, ServiceManager serviceManager) {
        if (ctx == null) {
            Log.m21e("Context cannot be null");
        }
