...
class DelayedHitSender implements HitSender {
    ...
    public boolean sendHit(String url) {
        if (!this.mRateLimiter.tokenAvailable()) {
            Log.m181w("Too many urls sent too quickly with the TagManagerSender, rate limiting invoked.");
            return false;
        }
        if (!(this.mWrapperUrl == null || this.mWrapperQueryParameter == null)) {
            try {
                ...
                Log.m179v("Sending wrapped url hit: " + url);
            } catch (UnsupportedEncodingException e) {
                Log.m182w("Error wrapping URL for testing.", e);
                return false;
            }
        }
