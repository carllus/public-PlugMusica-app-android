...
public class Container {
    ...
    public boolean getBoolean(String key) {
        ...
        if (runtime == null) {
            Log.m175e("getBoolean called for closed container.");
        ...
        try {
            ...
            Log.m175e("Calling getBoolean() threw an exception: " + e.getMessage() + " Returning default value.");
            return Types.getDefaultBoolean().booleanValue();
        }
    }
    ...
    public double getDouble(String key) {
        Runtime runtime = getRuntime();
        if (runtime == null) {
            Log.m175e("getDouble called for closed container.");
        ...
        try {
            ...
            Log.m175e("Calling getDouble() threw an exception: " + e.getMessage() + " Returning default value.");
            return Types.getDefaultDouble().doubleValue();
        }
    }
    ...
    public long getLong(String key) {
        Runtime runtime = getRuntime();
        if (runtime == null) {
            Log.m175e("getLong called for closed container.");
            return Types.getDefaultInt64().longValue();
        }
        try {
            return Types.valueToInt64(runtime.evaluateMacroReference(key).getObject()).longValue();
        } catch (Exception e) {
            Log.m175e("Calling getLong() threw an exception: " + e.getMessage() + " Returning default value.");
            return Types.getDefaultInt64().longValue();
        }
    }
    ...
    public String getString(String key) {
        Runtime runtime = getRuntime();
        if (runtime == null) {
            Log.m175e("getString called for closed container.");
            return Types.getDefaultString();
        }
        try {
            return Types.valueToString(runtime.evaluateMacroReference(key).getObject());
        } catch (Exception e) {
            Log.m175e("Calling getString() threw an exception: " + e.getMessage() + " Returning default value.");
            return Types.getDefaultString();
        }
    }
    ...
    public synchronized void refresh() {
        if (getRuntime() == null) {
            Log.m181w("refresh called for closed container");
        } else {
            try {
                if (isDefaultContainerRefreshMode()) {
                    Log.m181w("Container is in DEFAULT_CONTAINER mode. Refresh request is ignored.");
                } else {
                    long currentTime = this.mClock.currentTimeMillis();
                    if (useAvailableToken(currentTime)) {
                        Log.m179v("Container refresh requested");
                        loadAfterDelay(0);
                        this.mLastRefreshMethodCalledTime = currentTime;
                    } else {
                        Log.m179v("Container refresh was called too often. Ignored.");
                    }
                }
            } catch (Exception e) {
                Log.m175e("Calling refresh() throws an exception: " + e.getMessage());
            }
        }
        return;
    }
    ...
    public synchronized void close() {
        try {
            ...
            Log.m175e("Calling close() threw an exception: " + e.getMessage());
        }
        this.mRuntime = null;
        return;
    }
    ...
    public synchronized void callRefreshSuccess(RefreshType refreshType) {
        Log.m179v("calling containerRefreshSuccess(" + refreshType + "): mUserCallback = " + this.mUserCallback);
    ...
    public synchronized void load(Callback callback, ResourceLoaderScheduler scheduler, final Clock clock) {
        ...
        this.mDiskLoadCallback = new LoadCallback<Resource.ResourceWithMetadata>() {
            ...
            public void onSuccess(Resource.ResourceWithMetadata proto) {
                ...
                if (Container.this.isDefault()) {
                    ...
                    Log.m179v("setting refresh time to saved time: " + proto.timeStamp);
            ...
            private RefreshFailure failureToRefreshFailure(LoadCallback.Failure failure) {
                switch (C09134.$SwitchMap$com$google$tagmanager$LoadCallback$Failure[failure.ordinal()]) {
                    case 1:
        ...
        if (isDefaultContainerRefreshMode()) {
            Log.m177i("Container is in DEFAULT_CONTAINER mode. Use default container.");
        } else {
            ...
            this.mNetworkLoadCallback = new LoadCallback<Serving.SupplementedResource>() {
                ...
                public void onSuccess(Serving.SupplementedResource supplementedResource) {
                    synchronized (Container.this) {
                        ...
                        Log.m179v("setting refresh time to current time: " + Container.this.mLastRefreshTime);
    ...
    public void initEvaluators(Serving.Resource resource) {
        try {
            ...
            Log.m175e("Not loading resource: " + resource + " because it is invalid: " + err.toString());
        }
    }
    ...
    private void createInitialContainer() {
        ...
        if (expandedResource == null) {
            Log.m181w("No default container found; creating an empty container.");
