...
class SimpleNetworkDispatcher implements Dispatcher {
    ...
    private final String userAgent = createUserAgentString("GoogleTagManager", "3.02", Build.VERSION.RELEASE, getUserAgentLanguage(Locale.getDefault()), Build.MODEL, Build.ID);
    ...
    public void dispatchHits(List<Hit> hits) {
        ...
        for (int i = 0; i < maxHits; i++) {
            ...
            URL url = getUrl(hit);
            if (url == null) {
                Log.m181w("No destination: discarding hit.");
                ...
                if (request == null) {
                    ...
                    if (firstSend) {
                        try {
                            ...
                            Log.m181w("ClientProtocolException sending hit; discarding hit...");
                            this.dispatchListener.onHitPermanentDispatchFailure(hit);
                        } catch (IOException e2) {
                            Log.m181w("Exception sending hit: " + e2.getClass().getSimpleName());
                            Log.m181w(e2.getMessage());
                    ...
                    if (statusCode != 200) {
                        Log.m181w("Bad response: " + response.getStatusLine().getStatusCode());
                        this.dispatchListener.onHitTransientDispatchFailure(hit);
                    } else {
    ...
    private HttpEntityEnclosingRequest constructGtmRequest(URL url) {
        ...
        try {
            ...
            try {
                ...
                e = e;
                request = request2;
                Log.m181w("Exception sending hit: " + e.getClass().getSimpleName());
                Log.m181w(e.getMessage());
                return request;
            }
    ...
    private void logDebugInformation(HttpEntityEnclosingRequest request) {
        ...
        if (request.getEntity() != null) {
            try {
                ...
                Log.m179v("Error Writing hit to log...");
            }
        }
        Log.m179v(httpHeaders.toString());
    }
    ...
    public URL getUrl(Hit hit) {
        try {
            ...
            Log.m175e("Error trying to parse the GTM url.");
            return null;
        }
    }
}
