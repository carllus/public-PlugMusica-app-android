...
class JsonUtils {
    ...
    public static ResourceUtil.ExpandedResource expandedResourceFromJsonString(String jsonString) throws JSONException {
        ...
        for (int i = 0; i < value.mapKey.length; i++) {
            builder.addMacro(ResourceUtil.ExpandedFunctionCall.newBuilder().addProperty(Key.INSTANCE_NAME.toString(), value.mapKey[i]).addProperty(Key.FUNCTION.toString(), Types.functionIdToValue(ConstantMacro.getFunctionId())).addProperty(ConstantMacro.getValueKey(), value.mapValue[i]).build());
        }
